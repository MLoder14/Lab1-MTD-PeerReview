...
namespace MTDUserInterface
{
    public partial class PlayMTDRightClick : Form
    {
        ...
        //creates our new boneyard
        BoneYard boneyard;
        //create our player and computer hands
        Hand computerhand, playerhand;

        //MexicanTrain mexicantrain;
        PlayerTrain computerstrain, playertrain;
        ...
        public void SetUp()
        {
            ...
            playerhand = new Hand(boneyard, 2);

            int cDIndex = computerhand.IndexOfHighDouble();
            int pDIndex = playerhand.IndexOfHighDouble();

            if (playerhand[pDIndex].Side1 > computerhand[cDIndex].Side1)
            {
                //player goes first
                Domino highestDouble = playerhand[pDIndex];
                playerhand.RemoveAt(pDIndex);
